...
app.post('/signup', (req, res) =>{
    ...
    const sql1 = 'SELECT * FROM users WHERE  userEmail = ?';
    connection.query(sql1, values, (err, results) => {
        ...
        if (results.length === 0){
            ...
            const query2 = `INSERT INTO users (userPassword, userEmail) VALUES (?, ?);`;
...
app.post('/login', (req, res) => {
    ...
    const query1 = 'SELECT * FROM users WHERE userEmail = ?'
...
app.get('/todo', auth, TodoController.getTodo);
app.post('/todo', auth, TodoController.createTodo)
